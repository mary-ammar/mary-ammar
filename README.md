...
End-to-end ML pipeline on 1,000 observations: data cleaning, encoding, feature engineering, and class imbalance handling with SMOTE. Compared 4 models (Logistic Regression, Random Forest, XGBoost, SVM) using 5-fold cross-validation. Model explainability with SHAP values to identify the most impactful features on default risk.
...
**B2A Labs** — Processed 108,697 real STI screening records via SQL. Built GLM/GBM/XGBoost models (AUC 0.70), ran PCA on 57 labs, and deployed a production RShiny dashboard for weekly epidemiological monitoring.

**IBMP — CNRS** — Structured experimental data, applied statistical tests (Student, Mann-Whitney), and ensured full reproducibility via Git.
